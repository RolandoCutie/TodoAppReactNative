...
class Fire {
    ...
    async init() {
        ...
        try {
            ...
            if (storedUserId) {
                ...
                // Ensure device ID is set in Firebase
    ...
    getLists(callback: (lists: List[]) => void) {
        ...
        onValue(listsRef, (snapshot) => {
            if (!snapshot.exists()) {
                console.log("No lists found in Firebase.");
                callback([]);
                return;
            }
            ...
            snapshot.forEach((child) => {
                ...
                if (listData && typeof listData === 'object') {
                    lists.push({
                        id: getLists(callback: (lists: List[]) => void) {
                            if (!this.userId) {
                                console.log("No user ID available yet");
                                callback([]);
                                return;
                            ...
                            onValue(listsRef, (snapshot) => {
                                ...
                                snapshot.forEach((child) => {
                                    ...
                                    if (listData && typeof listData === 'object') {
                                        lists.push({
                                            id: child.key,  // No convertirlo a `parseInt`, Firebase usa strings como keys
                                            name: listData.name || "",
                        ...
                        },  // No convertirlo a `parseInt`, Firebase usa strings como keys
                        name: listData.name || "",
                        color: listData.color || "#000000",
                        todos: listData.todos || []
                    });
